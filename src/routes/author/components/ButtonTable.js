...
class ButtonTable extends PureComponent {
  /**
   * 获取行
   * @returns {*[]}
   */
    ...
    const rowSelection = {
      ...
      onSelect: (record, selected) => {
        ...
        if (selected) {
          ...
          //   const tree = arrayToTree(dataSource, 'id', 'parentId', 'children');
  ...
  onExpand = (expanded, record) => {
    ...
    dispatch({type: 'adminButton/update', payload: {expandedRowKeys}});
  }

  onFilter = (source = []) => {
    const {filterValue} = this.props;
    const filterMap = {};
    return source.filter(({id, title = '', children = []}) => {
      if (title.indexOf(filterValue) !== -1) {
        filterMap[id] = true;
      } else {
        filterMap[id] = false;
      }
      this.setFilterResult(children, id, filterValue, filterMap);
      return filterMap[id];
    });
  }

  setFilterResult = (childrenArray = [], id, filterValue = '', filterMap) => {
    for (const {title = '', children = []} of (childrenArray || [])) {
      if (title.indexOf(filterValue) !== -1) {
  ...
  render() {
    const {dataSource, size: {height}, tableKey = ''} = this.props;
    const data = this.onFilter(arrayToTree(dataSource || [], 'id', 'parentId'));
    const y = height ? height - 160 + 16 - 30 : document.body.clientHeight - 260;
    return (
      <Table
        key={tableKey}
        bordered
...
function mapStateToProps(state) {
  return {
    ...
    expandedRowKeys: state.adminButton.expandedRowKeys,
    filterValue: state.adminButton.filterValue,
    tableKey: state.adminButton.tableKey,
  };
}
